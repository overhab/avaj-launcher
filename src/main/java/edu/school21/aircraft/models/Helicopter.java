...
public class Helicopter extends Aircraft implements Flyable {
    ...
    public void updateConditions() throws IOException {
        ...
        if (weather.equals("RAIN")) {
            Simulation.LogSimulation("Helicopter#" + name + "(" + id + "): raining...");
            coordinates.setLongitude(coordinates.getLongitude() + 10);
        } else if (weather.equals("FOG")) {
            Simulation.LogSimulation("Helicopter#" + name + "(" + id + "): fog, can't see");
            coordinates.setLongitude(coordinates.getLongitude() + 1);
        } else if (weather.equals("SUN")) {
            Simulation.LogSimulation("Helicopter#" + name + "(" + id + "): sunny");
            coordinates.setLongitude(coordinates.getLongitude() + 10);
            int newHeight = coordinates.getHeight() + 2;
            newHeight = Math.min(newHeight, 100);
            coordinates.setHeight(newHeight);
        } else if (weather.equals("SNOW")) {
            Simulation.LogSimulation("Helicopter#" + name + "(" + id + "): snow");
